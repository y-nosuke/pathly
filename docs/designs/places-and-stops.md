...
**立ち寄りの検出は「記録中（自動）」の一度きり**で行う。命名は記録中（オンライン時・自動）に加え、
取りこぼした分を詳細画面の「場所を取得」で手動再実行できる。**詳細画面を開いても自動検出はしない**
（「stops が0件」では“本当に立ち寄りが無い”のか“取りこぼした”のか区別できず、勝手に叩くのは筋が悪いため）。
旧 `ensureStopsDetected`（開いたら冪等検出）は廃止した。生データから作り直す「再解析」も廃止した（下記）。
...
> **「再解析」ボタンは廃止した。** 立ち寄り検出は記録中に自動で走り、オンラインならその場で命名まで終わる。
> オフラインで命名できなかった分は「場所を取得」で拾えるので、**生データから作り直す破壊的な再検出はユーザーの出番が無い**。
> むしろ、誤検知として**手で削除した立ち寄りが再検出で復活する**（生データには滞在の痕跡が残るため）副作用があり、キュレーションと真っ向から衝突する。
> 検出は「記録中の一度きりの生成」と位置づけ、その後はユーザーのキュレーション（削除／将来の手動追加）を優先する。
> しきい値・アルゴリズムを変えたときの既存トラックへの反映は開発時の一回きりの関心事で、ユーザー向け導線には載せない。
>
> 取りこぼした立ち寄りの救済は、削除の“対”になる**手動追加（地図タップで立ち寄りを足す）**で将来対応する（非破壊・キュレーション的）。→「将来の拡張」を参照。
...
- **立ち寄りの削除**: 各行の「削除」で1件、行を**長押し**すると複数選択して一括削除できる（記録中に拾った誤検知の掃除を想定）。削除は単体・複数とも同じロジック（`deleteStops(stopIds)`）で、常に**訪問（stop）を消す**動作。消した結果**どこからも参照されなくなった place だけ**を自動で場所ごと回収する（孤立 GC）。回収条件は「**残る stop がゼロ かつ wishlist 登録もゼロ**」。逆に、他に stop が残る（＝他の履歴でも使われている）place や、wishlist に登録がある place は残す。**wishlist は place を消すと CASCADE で消える**ため、行きたい登録のある place は履歴画面からは巻き込まず必ず残す。place ごと明示的に消したいときは場所タブ（[wishlist.md](./wishlist.md)）から。
...
記録中の自動検出が取りこぼした立ち寄りを、ユーザーが**手で足せる**ようにする。誤検知を消す「削除」の“対”になる、
**非破壊・キュレーション的**な救済で、廃止した「再解析」（生データから全部作り直す破壊的操作）の置き換え。

- 詳細画面の地図で位置（できれば POI）を選び、到着／出発（または滞在時間）を指定して `stop` を1件作る。
- place は既存の重複排除（30m の `findOrCreatePlace`）に乗せ、既に手動登録した place（POIタップ登録）とも共有する。
- 生データを再走査しないので、**手で削除した立ち寄りが復活することはない**。
...
| 立ち寄りの手動追加(予定) | 詳細画面（削除の“対”・非破壊）→ 「将来の拡張」参照                                |
...
9. **再解析**ボタンを廃止（記録中の自動検出＋「場所を取得」で足り、削除した立ち寄りが復活する副作用を排除）
