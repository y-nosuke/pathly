...
- **再解析（追加提案・非破壊）**: その track を検出し直し（保存済みの補正点列に `StopDetector`）、
  **既存の立ち寄りと時間帯が重ならない候補だけ**を「一覧に無い立ち寄り」としてダイアログに出す。
  ユーザーが**選択式でチェックした分だけ**を追加保存する（`findOrCreatePlace` で place を確保＝命名済みは再利用、
...
- **再解析**ボタン: 終了済みの経路（2点以上）で表示 → 押すと「一覧に無い立ち寄り」候補を**選択式ダイアログ**で出し、チェックした分だけ追加する（非破壊）。候補ゼロなら「見つかりませんでした」。誤って削除した立ち寄りの復旧・取りこぼしの追加に使う
