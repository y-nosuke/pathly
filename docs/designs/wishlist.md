...
- **並び替え**: 行きたいを上に、優先度（高→低）→更新の新しい順。
- **フィルタ**: すべて / 行きたい / 訪問済み。
- **各行**: 場所名（未命名は住所→座標）・行きたいフラグ（タップで付け外し）・優先度（行きたい時）・メモ抜粋・訪問済みバッジ。
- **追加（＋）**: 地図タップで登録（下記）。
- 空のときは「場所がありません」＋追加導線。
...
- 行きたいフラグの付け外し。ON のときだけ優先度・メモ・訪問済みを編集できる。
- 場所そのものの削除はこの画面では行わない（places は再利用・立ち寄りと共有のため静的に保つ）。
...
- **一覧のノイズ**: 全 places を出すため、立ち寄り検出の未命名の点が多数並ぶ可能性。実機で量を見て、必要なら「名前あり優先」等の見せ方を検討。
- **場所の削除**: 場所そのものの削除は「場所」タブでは提供しない（places は立ち寄りと共有・静的に保つ）。行きたいの付け外しのみ。削除導線が要るなら別途設計。
