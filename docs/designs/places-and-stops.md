...
`LocationTrackingService` が新しい点を保存・補正する流れに続けて、立ち寄りも増分で検出・保存する。
補正（[gps-smoothing.md](./gps-smoothing.md)）と同じ「確定プレフィックス」の考え方を使う。

```
新しい点が保存・補正される
  └─ updateStopsForTrack(trackId, isFinal)
       1. StopDetector.detect(保存済みの補正点列) で DetectedStop 群を得る
       2. 離れて確定した立ち寄りだけを対象にする（案A）
          - 最後のクラスタが末尾点を含むうちは「滞在中」＝暫定。保存しない
          - isFinal（記録終了）なら末尾も確定
       3. 未保存の確定ぶんだけ findOrCreatePlace（30m重複排除）→ stops に保存
       4. オンラインなら resolveNames(trackId): 未解決の place を Places で命名
          （オフラインなら叩かない → 次にオンラインで確定が出たときキャッチアップ）
```

- **案A（確定のみ）**: 滞在中のライブ「立ち寄り中」表示は後段の拡張。まずは離れて確定したものだけ保存する。
...
記録中に3分を超えた滞在を「立ち寄り中」として暫定表示する。これは**DBに保存せずメモリ保持**でよい
（記録中サービスの `StateFlow` 等で現在の滞在を持ち、UIに流す）。確定した立ち寄りだけ DB に保存する
案Aと両立し、スキーマ変更（`departureTime` の nullable 化等）は不要。案Aで始めてから追加する。
...
5. 記録中の増分検出・保存（`updateStopsForTrack`、案A＝確定のみ）
