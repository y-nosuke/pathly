...
- **名前で探して足す**: `presentation/common/PlaceNameSearchField`（キーワード検索の共通部品）で
  施設を選べる。座標だけを頼りにすると、地図に POI が出ていない場所・代表点が敷地の中心にある広い施設は
  選べないため。出す場所は 2 つ。
  - **地点を指す前の案内（`ManualPickPrompt`）**: 地図をタップせずに名前だけで足せる。選んだ施設を
    `ManualStopOrigin.Poi` として `manualPick` に入れるので、**その施設をタップしたのと同じ状態**になる
    （到着・出発の推定も区間の調整も同じ経路を通る）。地図もその施設へ寄せる。
  - **入力シート（`ManualStopSheet`）**: 施設がまだ決まっていない起点（地図の空き地点・「今ここ」）だけ。
    POI 起点・既存の場所への紐付けは施設が確定済みなので出さない。
    どちらも選ぶと `googlePlaceId` と**施設の座標**が確定する。
