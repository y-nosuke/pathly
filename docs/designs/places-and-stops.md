...
- **課金**: Nearby Search は有料。呼び出しは「**詳細画面を開いたときに、まだ名前の無い場所だけ**」に限定してコストを抑える（下記トリガー参照）
...
TrackDetail を開く
  └─ ensureStopsDetected(track)
       ...
       └─ resolveMissingNames(trackId): name が null の場所だけ Places で命名（ベストエフォート）
  └─ getStopsForTrack(trackId) を Flow で購読して表示
```

この方式の利点:

- **既存の記録**（終了済みトラック）も、開いた時点で自動的に確定される（マイグレーション不要）
- Places 呼び出しは**閲覧した経路の未命名の場所だけ**＝課金を最小化
- 検出は補正後の点列に対して1度だけ。以後は保存済みを表示（軽量）

> 記録終了時に即確定しないのは、(1) その瞬間にネットワークが無いことが多い、(2) 見ない経路にまで課金したくない、ため。将来必要なら終了時トリガーも追加できる。
