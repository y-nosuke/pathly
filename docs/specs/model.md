...
- **バージョン**: 4
  - v2: places / stops を追加（[places-and-stops 設計](../designs/places-and-stops.md)）
  - v3: smoothed_points を追加（[gps-smoothing 設計](../designs/gps-smoothing.md)）
  - v4: place_resolutions（Google解決ログ）を追加（[places-and-stops 設計](../designs/places-and-stops.md)）
- **Room バージョン**: 2.8.4

> 注: 本書の一部は初期案（TEXT UUID 主キー等）で、実装は Long 主キー（`autoGenerate`）。
> 立ち寄り・場所の実データモデルは [places-and-stops.md](../designs/places-and-stops.md) を参照。
